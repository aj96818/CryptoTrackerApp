...
select sum(quantity) from accounts where symbol in ('eth');


select * from txns order by txn_id desc;

DELIMITER //
CREATE PROCEDURE sp_GetAvgBuySellAll()
BEGIN
SELECT
    t.symbol
    , t.txn_type
...
call sp_GetAvgBuySellAll;
	
select * from accounts where symbol = 'rune';

select * from accounts order by symbol desc;

select symbol, crypto_name, sum(quantity)
from accounts
group by symbol, crypto_name
order by crypto_name asc;

select * from txns where symbol = 'btc' order by txn_date desc;

select avg(price) from txns
where symbol = 'btc'
and cast(txn_date as date) between '2021-01-01' and '2021-12-31'
and txn_type = 'buy';

select * from txns where cast(txn_date as date) = '2021-02-07';
